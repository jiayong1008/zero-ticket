    ...
    <div className={`w-full flex flex-col h-[100dvh] min-h-0 overflow-hidden transition-colors duration-300 ${isLightMode ? "bg-slate-100" : "bg-[#0b0f19]"}`}>
      ...
      <div className="flex flex-1 overflow-hidden min-h-0">
        ...
        <aside className={`w-[25%] min-w-[250px] max-w-[320px] border-r h-full p-5 overflow-y-auto space-y-6 flex flex-col transition-colors duration-300 min-h-0 ${isLightMode ? "bg-slate-50 border-slate-200" : "bg-[#0f172a]/30 border-white/10"}`}>
          <div>
            <h2 className={`text-xs font-bold uppercase tracking-wider mb-1 flex items-center gap-2 transition-colors ${isLightMode ? "text-slate-500" : "text-slate-400"}`}>
              <svg className="w-4 h-4 text-indigo-500" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                <path strokeLinecap="round" strokeLinejoin="round" d="M19 11H5m14 0a2 2 0 012 2v6a2 2 0 01-2 2H5a2 2 0 01-2-2v-6a2 2 0 012-2m14 0V9a2 2 0 00-2-2M5 11V9a2 2 0 012-2m0 0V5a2 2 0 012-2h6a2 2 0 012 2v2M7 7h10" />
              </svg>
              Quick Demo Scenarios
            </h2>
            <p className={`text-[9.5px] leading-normal mb-3 transition-colors ${isLightMode ? "text-slate-400" : "text-slate-500"}`}>
              💡 Presets are shortcuts to set JWT claims and query inputs. The engine operates dynamically on any custom user query.
            </p>
